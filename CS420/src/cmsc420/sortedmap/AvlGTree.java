...
public class AvlGTree<K, V> extends AbstractMap<K, V> implements
    ...
    public V remove(Object key) {
        V retVal = get((K) key);
        this.root = deleteNode(this.root, (K) key);
        this.size--;
        this.modCount++;
        return retVal;
    }

    // source: http://www.geeksforgeeks.org/avl-tree-set-2-deletion/
    private AvlNode<K, V> deleteNode(AvlNode root, K key) {
    ...
    public static final class AvlNode<K, V> implements Map.Entry<K, V> {
        private K key;
        private V value;
